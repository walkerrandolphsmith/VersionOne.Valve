...
module.exports = class ValveRunner extends Runner {
    constructor(options) {
        super(options);
        this.memberName = options.member;
    }

    async command() {
        const v1 = this.authenticateAs('admin', 'admin');
        const memberName = this.memberName;
        const memberOid = await v1.create('Member', {
            Name: memberName,
            Username: memberName,
            Password: memberName,
        ...
        }).then(member => dropMoment(member.id));
        ...
        return v1.setMemberRoles(relativeUrl, payload).then(res => {
            console.log(`Member ${memberName} created with ${ROOT_SCOPE} Project Role.`);
        });
    }
};